            ...
            steps {
                script {
                    withCredentials([sshUserPrivateKey(credentialsId: '13.38.120.217', keyFileVariable: 'UBUNTU')]) {
                        ...
                        sh """ssh -o StrictHostKeyChecking=no -i \${UBUNTU} ubuntu@ec2-13-38-120-217.eu-west-3.compute.amazonaws.com 'docker run --name montest-d -p 8087:5000 mrad90/ic-webapp'"""
